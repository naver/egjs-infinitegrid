...
class InfiniteGrid<Options extends InfiniteGridOptions = InfiniteGridOptions> extends Component<InfiniteGridEvents> {
   ...
   * <ko>추가할 아이템에 설정할 그룹 키. 생략하면 값이 자동으로 생성된다.</ko>
   */
  public appendPlaceholders(items: number | InfiniteGridItemStatus[], groupKey?: string | number): this {
    this.groupManager.appendPlaceholders(items, groupKey);
    return this;
  }
  /**
   ...
   * <ko>추가할 아이템에 설정할 그룹 키. 생략하면 값이 자동으로 생성된다.</ko>
   */
  public prependPlaceholders(items: number | InfiniteGridItemStatus[], groupKey?: string | number): this {
    this.groupManager.prependPlaceholders(items, groupKey);
    return this;
  }
