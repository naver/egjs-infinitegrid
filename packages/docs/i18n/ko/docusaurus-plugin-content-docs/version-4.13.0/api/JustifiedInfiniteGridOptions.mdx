...
|columnRange|number \| number[] \| ((self: JustifiedGrid) =&gt; number \| number[])|[1, 8]| 한 줄에 들어가는 아이템의 최소, 최대 개수.|
...
|stretchRange|Array&lt;string \| number&gt;|["-10%", "+10%"]|string 값으로 <code>-</code>, <code>+</code>, <code>%</code>이 붙으면 원본 크기에 대한 상대값이며 number 값으로 들어오면 절대 값으로 stretch 범위를 설정할 수 있습니다.<br />각 아이템의 Element 또는 JSX에 <code>data-grid-min-stretch</code>, <code>data-grid-max-stretch</code>을 설정하면 우선 적용한다.|
