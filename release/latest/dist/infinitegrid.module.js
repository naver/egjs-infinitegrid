...
function isVisible(group, _ref) {
	var threshold = _ref.threshold,
	    scrollPos = _ref.scrollPos,
	    endScrollPos = _ref.endScrollPos;

	var start = group.outlines.start;
	var end = group.outlines.end;
...
var Infinite = function () {
	...
	Infinite.prototype.recycle = function recycle(scrollPos, isForward) {
		...
		var visibleOptions = { threshold: threshold, scrollPos: scrollPos, endScrollPos: endScrollPos };
		var visibles = this._items.get(startCursor, endCursor).map(function (group) {
			return isVisible(group, visibleOptions);
		});
	...
	Infinite.prototype._insertData = function _insertData(item, isAppend) {
	...
	Infinite.prototype.append = function append(item) {
		this._insertData(item, _consts.APPEND);
	};

	Infinite.prototype.prepend = function prepend(item) {
		this._insertData(item, _consts.PREPEND);
	};
...
exports.__esModule = true;
exports.AutoSizer = exports.ImageLoaded = exports.Infinite = exports.JustifiedLayout = exports.PackingLayout = exports.SquareLayout = exports.FrameLayout = exports.GridLayout = undefined;
...
var _Infinite = __webpack_require__(5);
...
var _Watcher = __webpack_require__(12);
...
var InfiniteGrid = function (_Component) {
	...
	InfiniteGrid.prototype.layout = function layout() {
		...
		if (isRelayout) {
			if (this.options.isEqualSize) {
				...
				var startCursor = this._infinite.getCursor("start");
				var endCursor = this._infinite.getCursor("end");
  ...
  */
	...
	InfiniteGrid.prototype.setStatus = function setStatus(status) {
		...
		if (!status || !status.options || !status._status || !status._renderer || !status._items || !status._watcher || !status._infinite) {
			return this;
		}
  ...
  */
	...
	InfiniteGrid.prototype.moveTo = function moveTo(index) {
		...
		var isResize = data.outlines.start && data.outlines.start.length === 0;
		var startCursor = this._infinite.getCursor("start");
		var endCursor = this._infinite.getCursor("end");
		...
		if (isInCursor || !useRecycle || isEqualSize || !isResize) {
			var pos = item && item.rect[this.options.horizontal ? "left" : "top"];

			if (typeof pos === "undefined") {
				pos = Math.max.apply(Math, data.outlines.start);
			}
			var fit = Math.min.apply(Math, data.outlines.start);
	...
	InfiniteGrid.prototype._setData = function _setData(layouted) {
		var isAppend = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
	...
	InfiniteGrid.prototype._insertData = function _insertData(layouted) {
		var isAppend = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
	...
	InfiniteGrid.prototype._postImageLoaded = function _postImageLoaded(_ref3) {
		var fromCache = _ref3.fromCache,
		    layouted = _ref3.layouted,
		    items = _ref3.items,
		...
		this._postLayoutComplete({ layouted: layouted, isAppend: isAppend, isTrusted: isTrusted, moveItem: moveItem, useRecycle: false });
	};
   ...
   */
	...
	InfiniteGrid.prototype._postLayout = function _postLayout(_ref5) {
		...
		_ImageLoaded2["default"].check(items.map(function (item) {
			...
			complete: function complete() {
				...
				// no recycle
				_this4._postImageLoaded({ fromCache: fromCache, layouted: layouted, items: items, isAppend: isAppend, isTrusted: isTrusted, moveItem: moveItem });
			},
			error: function error(_ref6) {
				...
				// recycle
				_this4._postImageLoadedEnd(items, isAppend, removeTarget, replaceTarget);
			}
		});
		return this;
	};
	...
	InfiniteGrid.prototype._requestAppend = function _requestAppend(_ref7) {
		...
		if (cache) {
			this._postCache({ cache: cache, isAppend: _consts.APPEND, isTrusted: _consts.TRUSTED });
		} else {
			/**
    ...
    */
	...
	InfiniteGrid.prototype._requestPrepend = function _requestPrepend(_ref8) {
		...
		if (cache) {
			this._postCache({ cache: cache, isAppend: _consts.PREPEND, isTrusted: _consts.TRUSTED });
		} else {
			/**
...
var DOMRenderer = function () {
	...
	DOMRenderer.prototype._init = function _init(el) {
		...
		if (this.options.isOverflowScroll) {
			var target = this.options.horizontal ? ["X", "Y"] : ["Y", "X"];
			...
			this.container = this.options.horizontal && _consts.DEFENSE_BROWSER ? _defense(element) : element;
		} else {
			this.view = window;
	...
	DOMRenderer.prototype.setContainerSize = function setContainerSize(size) {
		if (!this.options.isOverflowScroll || this.options.horizontal && _consts.DEFENSE_BROWSER) {
	...
	DOMRenderer.prototype.resize = function resize() {
		...
		if (this.isNeededResize()) {
			this._size = {
				...
				view: horizontal ? (0, _utils.innerWidth)(this.view) : (0, _utils.innerHeight)(this.view),
				item: null
			};
			return true;
		} else {
			this._size.view = horizontal ? (0, _utils.innerWidth)(this.view) : (0, _utils.innerHeight)(this.view);
		}
		return false;
	};
